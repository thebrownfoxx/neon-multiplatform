...
@Serializable
data class GetMemberRequest(val id: MemberId) : WebSocketMessage(kClass = this::class)

@Serializable
data class GetMemberNotFound(val id: MemberId) : WebSocketMessage(
    kClass = GetMemberNotFound::class,
    description = "The member with the given id was not found",
)

@Serializable
data class GetMemberConnectionError(val id: MemberId) : WebSocketMessage(
    kClass = GetMemberConnectionError::class,
    description = "There was an error connecting to one of the components of the server",
)

@Serializable
data class GetMemberSuccessful(val member: Member) : WebSocketMessage(
