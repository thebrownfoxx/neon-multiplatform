...
class KtorSerializedWebSocketMessage(
    ...
    override val serializedValue = memberBlockContext("serializedValue") {
        runFailing { frame.data.toString(Charsets.UTF_8) }
            .mapError { DeserializationError }
    }

    override suspend fun getLabel() = memberBlockContext("getLabel") {
        runFailing { converter.deserialize<WebSocketMessageHeader>(frame).label }
            .mapError { DeserializationError }
    }

    override suspend fun getRequestId() = memberBlockContext("getRequestId") {
        runFailing { converter.deserialize<WebSocketMessageHeader>(frame).requestId }
            .mapError { DeserializationError }
    }

    override suspend fun deserialize(type: Type) = memberBlockContext("getLabel") {
        runFailing {
